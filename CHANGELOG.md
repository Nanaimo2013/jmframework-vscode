# Changelog

All notable changes to the JMFramework VS Code Extension will be documented in this file.

The format is based on [Keep a Changelog]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vKeep a Changelog...v0.5.0
[Semantic Versioning]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vSemantic Versioning...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0(https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Keep a Changelog]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vKeep a Changelog...v0.5.0
[Semantic Versioning]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vSemantic Versioning...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0(https://semver.org/spec/v2.0.0.html).

## [Keep a Changelog]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vKeep a Changelog...v0.5.0
[Semantic Versioning]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vSemantic Versioning...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0 - 2025-03-26

### Added
- None

### Changed
- None

### Deprecated
- None

### Removed
- None

### Fixed
- None

### Security
- None

## [Unreleased]

## [Keep a Changelog]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vKeep a Changelog...v0.5.0
[Semantic Versioning]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vSemantic Versioning...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0 - 2025-03-25

### Added
- None

### Changed
- Completely redesigned file icons with modern gradients and improved visibility
- New color-coded system for different file types:
  - HTML files: Blue gradient
  - CSS files: Purple gradient
  - JS files: Orange gradient
  - UI files: Red gradient
  - Config files: Green gradient
  - Template files: Cyan gradient
  - Module files: Deep Blue gradient
- Improved text rendering in file icons with better font and positioning
- Enhanced file icon design with consistent file shape and fold effect

### Deprecated
- None

### Removed
- Old monochrome file icons

### Fixed
- Icon text alignment and readability
- Inconsistent icon styling

### Security
- None

## [Keep a Changelog]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vKeep a Changelog...v0.5.0
[Semantic Versioning]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vSemantic Versioning...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0 - 2025-03-25

### Added
- Initial release of JMFramework VS Code Extension
- Syntax highlighting for JMF files
- Code snippets for common JMF patterns
- Icon theme for JMF files and folders
- Basic language features (completion, hover)

### Changed
- None

### Deprecated
- None

### Removed
- None

### Fixed
- None

### Security
- None

## [Keep a Changelog]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vKeep a Changelog...v0.5.0
[Semantic Versioning]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vSemantic Versioning...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0 - 2025-03-25

### Added
- Support for .jmcss files
- Support for .jmhtml files
- Support for .jmconfig files
- Support for .jmtemplate files
- Support for .jmmodule files

### Changed
- Improved syntax highlighting
- Enhanced code completion
- Updated icon theme

### Fixed
- Various bug fixes and improvements

[Unreleased]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...HEAD
[Keep a Changelog]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vKeep a Changelog...v0.5.0
[Semantic Versioning]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vSemantic Versioning...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.4.0
[Keep a Changelog]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vKeep a Changelog...v0.5.0
[Semantic Versioning]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vSemantic Versioning...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.3.1
[Keep a Changelog]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vKeep a Changelog...v0.5.0
[Semantic Versioning]: https://github.com/Nanaimo2013/jmframework-vscode/compare/vSemantic Versioning...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0
[Unreleased]
[0.4.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.4.0...v0.5.0
[0.3.1]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.1...v0.5.0
[0.3.0]: https://github.com/Nanaimo2013/jmframework-vscode/compare/v0.3.0...v0.5.0: https://github.com/Nanaimo2013/jmframework-vscode/releases/tag/v0.3.0 